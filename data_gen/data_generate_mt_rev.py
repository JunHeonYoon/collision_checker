    ...
    def work(id, start_idx, end_idx, env_idx, result):
        ...
        for iter, q in enumerate(q_set[start_idx:end_idx, :]):
            ...
            if min_dist == -1: # collide
                q_idx_set.append(start_idx + iter)
                coll_set.append(1)
        ...
        if id == 0:
            ...
            dataset["num_q_step"] = np.array( [int( (joint_limit[1,k] - joint_limit[0,k]) / q_res ) for k in range(6)] + [int( (joint_limit[1,6] - joint_limit[0,6]) / (q_res*3) )] )


        dataset["depth"] = depth_set
        dataset["grid"] = voxel_set
        dataset["q_idx"] = q_idx_set
    ...
    for env_idx in range(args.num_env):
        ...
        for i in range(args.num_th):
            ...
            if id == 0:
                ...
                dataset["num_q_step"] = data["num_q_step"]
                dataset["depth"] = data["depth"]
                dataset["grid"] = data["grid"]
            dataset["q_idx"] = dataset["q_idx"] + data["q_idx"]
        ...
        with open(data_dir + "/dataset" + str(env_idx) +".pickle", "wb") as f:
            ...
            dataset["q_idx"] = np.array(dataset["q_idx"])
